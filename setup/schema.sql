...
CREATE TABLE `ActTechDisplay` (
  `ActTechDisplayId` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Primary Key',
...
CREATE TABLE `ActTechInfo` (
  `ActTechInfoId` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT 'Primary Key',
  ...
  `MusicPath` varchar(256) DEFAULT NULL,
...
CREATE TABLE `BidFeedback` (
  ...
  `ShowPref` SET( 'The Rhinestone Review', 'The Bordello', 'The Main Event (Competition)', 'The Main Event (Not in Competition)', 'The Newcomer\'s Showcase', 'The Sideshow Ball' )  NULL DEFAULT NULL,
...
CREATE TABLE `PreConRuns` (
  ...
  `Rooms` SET( "Crispus Attucks  (Master/Drop-in Classes)",
	"Haym Solomon (Food)",
